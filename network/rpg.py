...
class RPG:
    ...
    def _build_policy_network(self, state_in, name, reuse=False, batch_size=MINIBATCH_SIZE):
        ...
        with tf.variable_scope(name, reuse=reuse):
            lstm0 = tf.nn.rnn_cell.LSTMCell(num_units=RNN_UNIT_SIZE * 2, name='lstm_cell0')
            lstm = tf.nn.rnn_cell.LSTMCell(num_units=RNN_UNIT_SIZE, name='lstm_cell')
            lstm = tf.nn.rnn_cell.ResidualWrapper(lstm)
            lstm = tf.nn.rnn_cell.MultiRNNCell([lstm0] + [lstm] * (RNN_NUM_LAYERS - 1))
            ...
            logits = layers.dense(rnn_net, self.action_size,
                                  ...
                                  name='pi_logits')
            dist = tf.softmax(logits, name='action')
    ...
    def _build_baseline_network(self, state_in, name, reuse=False, batch_size=MINIBATCH_SIZE):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            critic = tf.reshape(critic, [batch_size, -1, 1])
    ...
    def _build_losses(self):
        with tf.variable_scope('loss'):
            ...
            with tf.variable_scope('policy'):
                ...
                obj_func = tf.log(tf.reduce_sum(self.result[0] * self.actions_OH, 1))  # log_pi(a|h)
                exp_v = tf.multiply(obj_func, td)
                traj_loss = tf.reduce_sum(exp_v, axis=1) / self.seq_len_
                self.policy_loss = -tf.reduce_mean(traj_loss, name='policy_loss')

            with tf.variable_scope('baseline'):
                err = self.target_result[1] - self.batch["rewards"]
                traj_err = tf.reduce_sum(tf.square(err), axis=1) / self.seq_len_
                self.baseline_loss = tf.reduce_mean(traj_err)
    ...
    def feed_forward(self, state, rnn_state):
        ...
        action = [np.random.choice(self.action_size, p=prob / sum(prob)) for prob in action_dist]

        return action, value, (p_fin_state, b_fin_state)
